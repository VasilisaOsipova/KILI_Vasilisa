...
# 1.3 Почему количество документов меньше количества вхождений? Ответ: потому что на одном документе может быть несколько вхождений
```
т
...
### Специальные символы  
о`*` : `нач*` задаст поиск всех лемм типа _начало, начать, начинать, начинить_. `*ча` задаст поиск всех лемм типа _чукча, буча, молча_  
...
# 3.3 Первое вхождение (лемма): цысарить цысарь (в тексте употреблено "цысарю")
```
...
```
№ 3.4. Ответ: потомуч что это не одно слово```
...
```
# 4.1 Самая последняя по алфавитному порядку словоформа: Бацочи
```
